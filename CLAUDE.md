...
- **Client IP for rate limits**: behind the tunnel, the peer the LXC-125 Caddy sees is cloudflared on LXC 139 (`10.10.88.39`) for every visitor, so without extra config every visitor shares one login/setup/webhook bucket. To fix that, the host-owned Caddyfile (`/opt/budget/caddy-etc/Caddyfile`) needs `trusted_proxies static 10.10.88.39/32` plus `client_ip_headers CF-Connecting-IP` in its global `servers` block and `header_up X-Real-IP {client_ip}` on the budget vhost, and `.env` needs `CLIENT_IP_HEADER=X-Real-IP`. The repo `Caddyfile` shows the pattern. Enable `CLIENT_IP_HEADER` only after Caddy overwrites that header; otherwise clients could pick their own bucket.
- **Containers**: `budget-app` (loopback `:3000`, healthcheck `/api/health`) + `budget-backup` (VACUUM cron, runs `sh /opt/budget-scripts/backup.sh` from a `./scripts` directory mount). The LXC-125 Caddy's config is **`/opt/budget/caddy-etc/Caddyfile`** (directory mount `./caddy-etc:/etc/caddy:ro`; the old `/opt/budget/Caddyfile` path is retired). It fronts budget for the tunnel hop AND trading/nextcloud `.sherrera.dev` and nine `*.bluefalls.home` vhosts — never rebuild/restart it casually; after an edit run `docker exec budget-caddy caddy validate --config /etc/caddy/Caddyfile` then `caddy reload --config /etc/caddy/Caddyfile`. Never mount single files into these containers: a single-file bind pins the inode at container start, so a replaced file silently never reaches the container (this detached Caddy from its config 2026-09-06 → 09-25 and kept `budget-backup` on a June `backup.sh`).
